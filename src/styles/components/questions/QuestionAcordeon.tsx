...
export const AccordeonTitle = styled.div<{ isactive: boolean }>`
  display: flex;
...
interface AccordeonContentProps {
  isactive: boolean; // Ajusta la interfaz para aceptar booleano o string
}
...
export const IconContainer = styled.div<IconContainerProps>`
  background-color: ${({ isactive }) => (isactive ? 'var(--theme)' : '#fff')};
  width: 30px;
  height: 30px;
  display: flex;
...
export const ExpandIcon = styled.div<ExpandIconProps>`
  color: ${({ isactive }) => (isactive ? 'var(--white)' : 'var(--theme)')};
  font-size: 1.2em;
